  ...
  setup(props) {
    ...
    const getBackground = computed(() => {
      ...
      if (imageFilesVersion.value.length > 0) {
        let imagesVersion = imageFilesVersion.value.filter((res) => res.startsWith('1'.repeat(8)))[0]
        if (imagesVersion) {
      ...
      if (imageFilesRegion.value.length > 0) {
        let imagesRegion = imageFilesRegion.value.filter((res) => res.startsWith('1'.repeat(8)))[0]
        if (imagesRegion) {
      ...
      if (imageFilesPlatform.value.length > 0) {
        let imagesPlatform = imageFilesPlatform.value.filter((res) => res.startsWith('1'.repeat(8)))[0]
        if (imagesPlatform) {
