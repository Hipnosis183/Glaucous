<template>
  <button
    class="bg-color-700 dark:bg-color-800 hover:bg-color-600 dark:hover:bg-color-700 px-4 py-2 shadow-color text-theme-200 whitespace-nowrap"
    :class="[
      buttonLarge ? 'w-full' : 'w-max',
      { 'h-12 text-xl' : buttonSize == 'small' },
      { 'h-16 text-2xl' : buttonSize == 'medium' },
      { 'h-20 text-4xl' : buttonSize == 'large' }
    ]"
  >
